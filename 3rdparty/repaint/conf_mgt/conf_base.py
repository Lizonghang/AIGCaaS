...
def write_images(imgs, img_names, dir_path, step=None, t_T=None):
    ...
    if type(imgs) is np.ndarray:
        ...
        assert step is not None and len(step) == len(imgs)

        base_name_ = img_names[0]
        base_path_ = os.path.join(dir_path, base_name_.replace(".png", ""))
        os.makedirs(base_path_, exist_ok=True)

        for i, image in zip(step, imgs):
            im_name_ = f"step-{i}.png"
            out_path = os.path.join(base_path_, im_name_)
            imwrite(img=image[0], path=out_path)
